...
**This issue has been fixed in the code** (as of the latest version). The application now uses an explicit JDBC URL that ignores environment variables.

If you still encounter this error with an older version, it means PostgreSQL is trying to use your system username instead of the configured credentials.

**If using an older version:**

```bash
# Check and unset PostgreSQL environment variables
env | grep PG
